...
conn = pymysql.connect(host='localhost', port=3306, user='root', password='', db='nhandienbiensoxe')
a = conn.cursor()
...
def BatCameRa(request):
    cap = cv2.VideoCapture(0) # truyền vào cảmera
    ...
    while True:
        ...
        if k == ord('\r'):
            ...
            cv2.imwrite("static/public/img/Camera/photo%d.jpg" % file_count, frame)
        ##Lấy chiều rộng và chiều cao của clip
        w = str(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        h = str(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        # Hiển thị thời gian lên camera
...
def nhandienAnh(request):
    if request.is_ajax and request.method == "POST":
        ...
        sql = "SELECT ma_guixe FROM tbl_guixe WHERE bienso_vao = '" + bienso + "' AND FK_giaxe = '6' OR FK_giaxe = '3' AND trangthai = 'Đang gửi xe'"
        a.execute(sql)
        ma_thang = a.fetchone()
        if ma_thang != None:
            ma = ma_thang
        ...
        if ma == None:
            ...
            sql = "UPDATE tbl_guixe SET trangthai = 'Đã trả xe', tg_kt_gui = " + (str)(time_now) + ", tongtien = " + (
                str)(giaxe) + " WHERE ma_guixe = " + (str)(maxe)
            a.execute(sql)
            conn.commit()
...
class dangkyxethang(View):
    ...
    def post(self, request):
        if request.POST['themkhachhang']:
            ...
            giaxe = (int)(request.POST['thongtingiaxe'])
            bienso = (str)(biensoxe)
            sql = "INSERT INTO tbl_guixe(bienso_vao, ngaygui, tg_bg_gui, FK_giaxe, id_khach) " \
                  "VALUES ('" + bienso + "','" + dt_string + "'," + (str)(time_now) + "," + (str)(giaxe) + ")"
            a.execute(sql)
            conn.commit()
